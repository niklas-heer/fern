#!/usr/bin/env python3
"""Exercise independent module namespaces through native execution and atomic rejection."""
import argparse
import os
...
FIXTURES = ROOT / "compiler-rs/tests/namespaces_native"
SENTINEL = b"existing executable\x00must survive failed namespace checking\xff"
...
def main():
    ...
    assert positive and negative, "namespace native corpus is missing"
    assert set(FIXTURES.glob("*.stdout")) == {source.with_suffix(".stdout") for source in positive}
    with tempfile.TemporaryDirectory(prefix="fern-namespace-native-") as temporary:
    ...
    print(f"Rust namespaces passed: {len(positive)} native programs, {len(negative)} atomic invalid programs")
