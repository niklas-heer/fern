#!/usr/bin/env python3
"""Check the MSRV lint contract through isolated, offline positive and negative crates."""
import os
...
REQUIRED = {"dbg_macro", "todo", "unimplemented", "exit", "unchecked_duration_subtraction", "unused_peekable", "redundant_clone", "or_fun_call"}
...
def policy():
    ...
    assert "unchecked_time_subtraction" not in tables, "unsupported MSRV lint spelling"
...
def run():
    ...
    failures = {
        ...
        "unchecked_duration_subtraction": "pub fn value(a:std::time::Instant,b:std::time::Duration)->std::time::Instant { a-b }",
    ...
    with tempfile.TemporaryDirectory(prefix="fern-lint-policy-") as temporary:
        ...
        for lint, source in failures.items():
            ...
            assert result.returncode != 0, f"{lint} was accepted"
        ...
        assert result.returncode == 0, result.stderr
    print(f"Rust lint policy: {len(failures)} forbidden cases, checked positive and test-only panic passed")
